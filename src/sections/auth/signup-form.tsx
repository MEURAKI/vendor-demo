...
import { supabase } from "../../lib/supabase/client"; // from src/sections/auth/ → ../../lib/...
...
export default function SignupForm() {
  ...
  const allGood =
    checks.length && checks.lower && checks.upper && checks.number && checks.special && formData.acceptTerms;
    ...
    <div className="min-h-screen bg-white flex">
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center px-6 sm:px-10 lg:px-16 py-10">
        <div className="w-full max-w-md">
          {/* Header */}
          <div className="mb-8">
            <h1 className="text-4xl sm:text-5xl font-extrabold text-black mb-1">Create an account</h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            {/* Email */}
            <div>
              <label className="text-black font-semibold tracking-wide text-sm">EMAIL ADDRESS</label>
              <input
                type="email"
            ...
            {/* Password */}
            <div>
              <label className="text-black font-semibold tracking-wide text-sm">PASSWORD</label>
              <input
                type="password"
                ...
                className={`mt-2 w-full h-14 rounded-2xl px-4
                           bg-[#EFEDFF] border
                           ${formData.password && !allGood ? "border-purple-300" : "border-transparent"}
                           text-gray-900 placeholder-gray-500
                           focus:outline-none focus:ring-2 focus:ring-purple-500`}
            ...
            <label className="flex items-start gap-3 text-sm text-gray-600">
              <input
                ...
                onChange={(e) => setFormData((s) => ({ ...s, acceptTerms: e.target.checked }))}
                className="mt-1 h-4 w-4 rounded border-gray-300 text-purple-600 focus:ring-purple-500"
              />
              <span>
                By clicking “Sign up”, you are creating a <b>MEURAKI Subscriber account</b> and therefore you agree to
                MEURAKI{" "}
            ...
            <button
              ...
              className={`w-full h-14 rounded-full text-base font-medium transition-colors shadow-lg
                         ${allGood ? "bg-black text-white hover:bg-gray-900" : "bg-gray-200 text-gray-500 cursor-not-allowed"}`}
            >
              Sign up
            </button>
            ...
            <p className="text-center text-xs text-gray-500 leading-relaxed">
              By clicking the “Sign up” button, you are creating a MEURAKI Subscriber account and therefore you agree to
              MEURAKI <Link href="/terms" className="underline">Terms of Use</Link> and{" "}
          ...
          <div className="mt-10 flex justify-center">
            ...
            {/* or "/logo-meuraki.svg" if that's where your file is */}
          </div>
        </div>
      </div>
      ...
      <div className="hidden lg:block lg:w-1/2 relative">
        ...
        <div className="absolute bottom-10 left-10 right-10 text-white">
          <p className="text-lg leading-relaxed font-medium max-w-[480px]">
            “Untitled Laboratory: a haven of avant-garde. With unceasing innovation,
            we transform abstract ideas into tangible reality.”
          </p>
  ...
  return (
    ...
    >
      <svg width="14" height="14" viewBox="0 0 24 24" aria-hidden="true">
        <path d="M9 16.2l-3.5-3.5-1.4 1.4L9 19 20.3 7.7l-1.4-1.4z" fill="currentColor" opacity={ok ? 1 : 0.45}/>
      </svg>
      {label}
    </span>
  );
