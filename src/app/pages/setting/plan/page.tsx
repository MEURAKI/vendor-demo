...
type VendorBusiness = {
  ...
  // NEW: address + google business id
...
function ShopSettingsPageInner() {
  ...
  useEffect(() => {
    (async () => {
      ...
      setTagsInput((merged.business_tags || []).join(", "));
      setLoading(false);
    })();
  }, []);
  ...
  function buildPayload(src: VendorBusiness) {
    return {
      ...
      shop_address: (src.shop_address || "").trim() || null,
      google_business_page_id: (src.google_business_page_id || "").trim() || null,
      business_tags: src.business_tags ?? [],
  ...
  async function save(tab: TabKey) {
    ...
    const withTags: VendorBusiness = { ...vb, business_tags: tags };

    const payload = buildPayload(withTags);
        ...
        <div className="mx-auto max-w-5xl px-8 py-10">
          ...
          {activeTab === "general" && (
            ...
            >
              ...
              {/* Shop Address + Google business id */}
              <section className="grid grid-cols-1 gap-6 sm:grid-cols-2">
                <div>
                  ...
                  <p className="mt-1 text-xs text-gray-500">
                    Start typing to enter your business location. (Google Place ID can be
                    stored here too.)
                  </p>
                </div>
                <div className="space-y-4">
                  <input
                    ...
                    placeholder="Type address, e.g. 56 Tanglin Road, 01-03 Singapore 247964"
                  />
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="(Optional) You can populate this from your onboarding / admin tools"
                    />
                  </div>
                </div>
              </section>
              ...
              <section className="grid grid-cols-1 gap-6 sm:grid-cols-2">
                <div>
                  ...
                  <p className="mt-1 text-xs text-gray-500">
                    Primary focus hashtags. Separate with commas. Example:{" "}
                    <span className="font-mono">#yoga, #yogamats</span>
                  </p>
                </div>
                <textarea
                  rows={2}
          ...
          {/* FULFILMENT TAB -------------------------------------------- */}
          {activeTab === "fulfilment" && (
            /* your existing fulfilment form unchanged */
            <form
              className="mt-8 space-y-12 pb-28"
              onSubmit={(e) => {
                e.preventDefault();
