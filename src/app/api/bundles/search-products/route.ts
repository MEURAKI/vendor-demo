...
export async function GET(req: Request) {
  ...
  const variantItems =
    variantData?.map((row: any) => {
      const productName = row.products?.name ?? "Untitled product";
      ...
      const variantImage =
        row.image_url ?? row.products?.image_url ?? null;

      return {
        id: String(row.id),              // local row id
        kind: "variant" as const,
        productId: String(row.products.id),
